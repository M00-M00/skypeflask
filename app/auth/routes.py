...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user_db is None or not user_db.check_password(form.password.data):
...
def register():
    ...
    if form.validate_on_submit():
       user_db  = user(username=form.username.data, email=form.email.data)
...
def reset_password(token):
    if current_user.is_authenticated:
        return redirect(url_for('main.index'))
    user_db  = user.verify_reset_password_token(token)
    if not user_db :
        return redirect(url_for('main.index'))
